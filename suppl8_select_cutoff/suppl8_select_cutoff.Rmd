...
In the following plot, basically a cutoff in the interval [0.72, 0.98] generates almost the same clusterings, and with cutoff 
lower than 0.71, the clusterings lead to more groups, which decreases the difference scores and increase the block mean values.
...
If we run `select_cutoff()`, actually we can see the cutoff can lead to two different clusterings (cutoff in [0.83, 0.98] and in [0.67, 0.82]).

```{r, fig.height = 5}
select_cutoff(mat, verbose = FALSE)
```

And if we set the cutof to some value lower than 0.83, the small dense clusters will be captured.
